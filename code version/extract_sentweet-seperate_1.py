...
#---------First version, contains getting words from urban dictionary---------------------------#
...
def groups_bag():
	groups = os.listdir("urban_dict")
	word_lists = []
	for group in groups:
		#-------------------pickle version---------------------#
		"""			
		if not ".pickle" in group:		#use pickle file to deal with tweets
			continue
		f = open("urban_dict/%s"%group,"rb")
		wlist = pickle.load(f)
		"""
		#-------------------txt version-----------------------#
		wlist = []
		if ".txt~" in group or ".pickle" in group:
			continue
		#print(group)
		wlist = [line.rstrip('\n') for line in open("urban_dict/%s"%group)]
...
def word_set(lists):
	...
	for l in lists:
		for w in l:
			...
			if w in word_freq:
				continue
	#-----------put word in a dictionary, key is word, value is freq---------#
			word_freq[w] = 0		#initialize this freq = 0
...
if __name__ == "__main__":
	ids = os.listdir('Data')
	word_lists = groups_bag()
	word_freq = word_set(word_lists)
	user_num = 1
